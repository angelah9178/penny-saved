...
@pytest.mark.parametrize(
    ("now", "months_range", "expected_start"),
    [
        (
...
def test_statistics_interval_subtracts_calendar_months_with_day_clamping(
    now: datetime,
    months_range: StatisticsRange,
    expected_start: datetime,
) -> None:
    interval = statistics_interval_for(months_range, now=now)
...
def test_statistics_interval_normalizes_non_utc_input_before_calculation() -> None:
    eastern_daylight_time = timezone(timedelta(hours=-4))
    local_time = datetime(2026, 8, 1, 0, 30, tzinfo=eastern_daylight_time)

    interval = statistics_interval_for(StatisticsRange.THIS_MONTH, now=local_time)

    assert interval == StatisticsInterval(
...
def test_statistics_interval_rejects_an_unrecognized_runtime_value() -> None:
    with pytest.raises(ValueError, match="Unsupported statistics range"):
        statistics_interval_for("quarter", now=datetime(2026, 8, 5, tzinfo=UTC))  # type: ignore[arg-type]
