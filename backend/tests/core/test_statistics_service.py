...
async def test_summary_reads_clock_once_and_passes_exact_range_to_repository(
    monkeypatch: pytest.MonkeyPatch,
    selected_range: StatisticsRange,
    expected_start: datetime | None,
) -> None:
    aggregate = StatisticsAggregate(25_000, 4, 1)
    repository = AsyncMock(return_value=aggregate)
...
@pytest.mark.asyncio
async def test_summary_preserves_zero_and_large_integer_aggregates(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
