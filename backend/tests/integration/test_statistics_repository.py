...
async def test_aggregate_applies_status_ownership_and_half_open_boundaries(
    ...
    db.add_all(
        [
            ...
            make_entry(status=EntryStatus.SAVED, checked_in_at=END, price_cents=9_999),
            make_entry(
...
async def test_all_time_has_no_start_but_still_excludes_the_end(db: AsyncSession) -> None:
    db.add_all(
        [
            make_entry(status=EntryStatus.SAVED, checked_in_at=START - timedelta(days=365)),
