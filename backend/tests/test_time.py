...
def test_normalize_utc_converts_aware_non_utc_time() -> None:
    ...
    assert normalize_utc(local_time) == datetime(2026, 1, 15, 12, 30, tzinfo=UTC)
    assert normalize_utc(local_time).tzinfo is UTC
...
def test_fixed_clock_normalizes_and_repeats_one_time() -> None:
...
def test_fastapi_can_override_clock_dependency() -> None:
