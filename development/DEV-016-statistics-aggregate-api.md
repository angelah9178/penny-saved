...
- Add an immutable half-open interval containing an optional start and required end.
...
Before this commit, the statistics service exists only inside the backend's Python
code and cannot be called directly by a browser.
...
When a client requests the URL, the backend confirms the user is logged in, validates
the range, calls the Commit 3 service, and returns its statistics as JSON. The route is
a safe doorway; it does not contain SQL, date arithmetic, or aggregation logic.

This still does not build the visible statistics page. DEV-018 calls this endpoint
from the frontend and renders the cards and filters.
...
- Update this tracker, commit statuses, hashes, implementation record, and actual test
